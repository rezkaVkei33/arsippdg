...
class AkademikMaster extends SistemNilai_Controller
{
    ...
    public function __construct() { parent::__construct(); $this->load->model('sistem_nilai/AkademikMaster_model', 'master_model'); $this->load->library('form_validation'); }
    ...
    private function listing($type) { $q = trim((string) $this->input->get('q', TRUE)); $this->render('master_akademik/index', ['title'=>$this->label($type).' - Sistem Nilai','type'=>$type,'label'=>$this->label($type),'items'=>$this->master_model->get_all($type,$q),'keyword'=>$q]); }
